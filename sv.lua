...
local function getVersionFromManifest()
    ...
    local file = io.open(path, "r")  -- Open the manifest file for reading
    if file then
        local content = file:read("*a")  -- Read the entire file
        file:close()

        for line in string.gmatch(content, "[^\r\n]+") do
            local key, value = string.match(line, "^version%s+'(.+)'$")
            if key then
                return value
            end
        end
    end
    return 'unknown'  -- Return 'unknown' if the version isn't found
end
...
function checkForUpdates()
    ...
    PerformHttpRequest(versionUrl, function(err, responseText, headers)
        if err == 200 then  -- HTTP status code 200 means the request was successful
            local data = json.decode(responseText)
            if data and data.latestVersion and data.latestVersion ~= currentVersion then
                print('Update available! Please download the latest version: ' .. data.latestVersion)
            else
